...
const consumer = kafka.consumer({
  groupId: process.env.AZURE_EVENT_HUB_CONSUMER_GROUP,
});
...
const startConsumer = async () => {
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      ...
      if (
        event.eventType === "doctor.status.changed" &&
        event.newStatus === "INACTIVE"
      ) {
        console.log(
          `Doctor inactivo: ${event.doctorId}, reprogamando citas...`,
        );
        await AppointmentService.handleDoctorInactive(
          event.doctorId,
          process.env.SECURITY_SERVICE_TOKEN,
        );
      }
    },
  });
